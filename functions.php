...
if ( ! function_exists( 'tatva_setup' ) ) {
	function tatva_setup() {
		...
		// Create an extra image size for the Home Post featured image
		add_image_size( 'post-thumb', 335, 252, true );
		
		// Create an extra image size for the Product image
		add_image_size( 'product-image', 335, 204, true );
...
function tatva_widgets_init() {
	...
	register_sidebar( array(
		...
		register_sidebar( array(
			'name' => esc_html__( 'Front-testimonial', 'tatva' ),
			'id' => 'testimonial',
			'description' => esc_html__( 'Appears in the footer sidebar', 'tatva' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
